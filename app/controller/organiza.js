...
class OrganizationController extends Controller{
 ...
 /*       let menu=await this.app.mysql.select('t_organization',{
    ...
    async saveAdd(){
        ...
        if(entity.is_leaf==1){
            await this.app.mysql.query('update t_organization set is_leaf=? where id=?',[0,entity.parent_id]);
        }
        ...
        result=await this.app.mysql.query('update t_organization set is_leaf=?,path=?  where id=?',[1,newPath,insert.insertId]);
