...
class MetadataController extends Controller{
    ...
    async queryMetadata(){
        ...
        let sql=`select * from isp_metadata
            where 1=1${c_systemId}${c_metadataType}${c_databaseType}${c_proType}`;
        console.log(sql);
    ...
    async metadataFields(){
        let result=await this.app.mysql.query(`select * from isp_metadata_fields where metadata_id=?`,[this.ctx.params.metadataId]);
        this.ctx.body=result;
    }
}
